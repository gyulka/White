    ...
    def __init__(self, group, bullets, obj, wall_h, wall_v, otkuda, kuda, sc, event, otraj=3):
        ...
        self.moving = [
            ...
            -self.velocity * math.cos(self.alfa) * (self.otkuda[1] - self.kuda[1]) // abs(
                self.otkuda[1] - self.kuda[1])]
        self.image = pygame.image.load('files/textures/mini_object/shoot1.png')
...
class Person(pygame.sprite.Sprite):
    ...
    def check_in_stop(self, character):
        ...
        sum = 0
        znach = None
        for i in range(len(self.boxes)):  # Проверка, в какой клетке находится игрок.
            txt = self.txt_level[i].split()
            cord = self.boxes[i]
            x1, y1 = character[0] + 2, character[1] + 80
            if (((x1 + 38 > cord[0] and x1 + 42 < cord[0] + 40) or (x1 > cord[0] and x1 < cord[0] - 40)) and (
                    y1 >= cord[1] and y1 <= cord[1] + 40)):
                sum += 1
